...
def simple_interest(principal, i_rate, years):
    interest = (principal * i_rate * years) / 100
    return interest
...
def convert_temperature(temp, scale):
    if scale == "C":
        ...
        print("Please enter either F or C to indicated scale")
...
def largest(int1, int2, int3):
    largest_so_far = int1
    if int2 >= largest_so_far:
        largest_so_far = int2
    if int3 >= largest_so_far:
        largest_so_far = int3
    return largest_so_far

print('Exercise 6:', largest(11, 9, 22))
...
def basicCalculator(num1, num2, operation):
    if operation == "add":
        ...
        ("Please add two numbers and an operation: add, subtract, muliply, divide." )
